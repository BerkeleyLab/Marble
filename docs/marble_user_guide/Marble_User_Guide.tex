...
\section{Ethernet}
Marble is equipped with Ethernet PHY (88E1512) which supports 10/100/1000BASE-T
(bedrock firmware only supports 1000BASE-T).
PHY is connected to the FPGA bank 16 via RGMII interface:
...
The IP and MAC addresses are stored in the MMC's internal EEPROM memory (EEPROM functionality is emulated in the internal flash).  After the FPGA boots, this information is passed to the FPGA over SPI. This feature depends, of course, on having a bitfile that implements it; this is a standard part of Bedrock's test\_marble\_family bitfiles.
...
\begin{enumerate}
  ...
  \item The watchdog hash values (see section XXX) are passed between the MMC and the FPGA via this mailbox interface
