...
The S25FL128SAGMFIR01 configuration memory is connected to the FPGA chip.
By default, the FPGA chip loads the configuration from the lowest address of flash memory after correct power cycle.
The module is equipped with a switch (fig \ref{fig:bootsw}) that blocks the programming of the configuration memory.
...
To write to configuration memory, an FPGA image must be loaded (e.g. via JTAG) which implements the chip's SPI
interface and acts as a bridge between the host computer (via USB, Ethernet, or some other interface) and
the flash IC.
Common flash memory interface commands are beyond the scope of this document, but an example implementation
of such a bridge is provided in ``bedrock/badger/spi\_flash.v''.
The default Marble image (``bedrock/projects/test\_marble\_family'') provides an interface to this bridge on Ethernet port 804
...
\section{Ethernet}
Marble is equipped with one Ethernet PHY chip (88E1512) which supports 10/100/1000BASE-T
(bedrock firmware only supports 1000BASE-T) and connects to a network via RJ45 connector J4
and to the FPGA bank 16 via RGMII interface:
