...
\multirow{2}{*}{System Clock} &
  Y1, Y2 &
  \multirow{2}{9cm}{Y1 and Y2 can be soldered interchangeably and provide a reference clock source for the U20 chip} \\
 &
  U20 &
  \multirow{2}{9cm}{U20 is an ultra-low jitter clock generator that provides a 125 MHz clock for the FPGA bank 33 that can operate as a DDR3 reference clock. Additionally, U20 generates 125 MHz clock which is connected to the clock multiplexer. }
   \\
Additional Clock &
  Y3 &
  VCXO - provides 20 MHz clock for FPGA bank 33 \\
User Clock &
  Y6 &
  Si570 - provides variable clock frequencies for 10 MHz to 280 MHz; connected to the clock multiplexer \\
...
\label{tab:rgmii}
\end{table}
The Ethernet PHY can be monitored over MDIO by the MMC. IP address and MAC address are stored in the MMC's internal EEPROM memory (EEPROM functionality is emulated in the internal flash).
